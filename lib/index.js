...
/** 计价配置（用户覆盖表），apply() 启动时注入，collectUsage 消费。 */
let priceConfig = {};
...
function serializeSession(state) {
	...
	for (const [date, entry] of state.days) {
		...
		days[date] = { totals: { ...entry.totals }, cost: entry.cost ?? 0, models };
	}
	return {
...
function parseSession(raw) {
	...
	if (raw.days !== null && typeof raw.days === "object") {
		for (const [date, entry] of Object.entries(raw.days)) {
			...
			const target = { totals: zeroBuckets(), cost: 0, models: new Map() };
			target.cost = Number.isFinite(entry.cost) ? entry.cost : 0;
	...
	if (raw.lastSample !== null && raw.lastSample !== void 0 && typeof raw.lastSample === "object" && typeof raw.lastSample.key === "string" && typeof raw.lastSample.day === "string") {
		...
		state.lastSample = {
			...
			cost: Number.isFinite(raw.lastSample.cost) ? raw.lastSample.cost : 0
		};
	}
...
async function loadCache() {
	...
	loadPromise ??= (async () => {
		const fresh = { version: CACHE_VERSION, sessions: {}, dailyBalance: {} };
		try {
			...
			if (parsed !== null && typeof parsed === "object" && parsed.version === CACHE_VERSION && parsed.sessions !== null && typeof parsed.sessions === "object") {
				...
				return {
					...
					sessions,
					dailyBalance: parsed.dailyBalance !== null && typeof parsed.dailyBalance === "object" ? parsed.dailyBalance : {}
				};
			}
		} catch {
...
async function saveCache(ctx, cache) {
	try {
		...
		const serialized = { version: CACHE_VERSION, sessions: {}, dailyBalance: cache.dailyBalance ?? {} };
...
export async function collectUsage(ctx) {
	return withLock(async () => {
		...
		const price = (model, timeMs, buckets) => costOf(model, timeMs, buckets, priceConfig);
		...
		if (live !== void 0) {
			for (const session of live.list()) {
				...
				if ((state.consumed ?? 0) < count) {
					applyUsageDelta(state, session.events.slice(state.consumed ?? 0), price);
...
//#region daily balance snapshots
/** Local `YYYY-MM-DD`. */
function balanceDayKey() {
	const now = new Date();
	return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
}

/**
 * Record the day's latest balance snapshot (overwrites within the same day so
 * the stored value is the last seen balance ≈ end-of-day when the panel was
 * open). Keeps only the latest 45 days.
 */
function noteDailyBalance(cache, account) {
	if (account === null || account?.status !== "ok" || account?.balance?.remaining == null) return;
	cache.dailyBalance ??= {};
	const day = balanceDayKey();
	const b = account.balance;
	cache.dailyBalance[day] = {
		remaining: Number(b.remaining) || 0,
		toppedUp: Number(b.breakdown?.toppedUp) || 0,
		granted: Number(b.breakdown?.granted) || 0,
		ts: Date.now()
	};
	const keys = Object.keys(cache.dailyBalance).sort();
	for (let i = 0; i < keys.length - 45; i += 1) delete cache.dailyBalance[keys[i]];
}

/**
 * All-channel spend estimate via balance delta: previous day's last snapshot
 * minus the current remaining, minus any recharge (toppedUp/granted growth).
 * Captures web/App/other-client usage the session logs cannot see. Returns
 * null until a previous-day snapshot exists.
 */
function allChannelSpendEstimate(cache, account) {
	if (account === null || account?.status !== "ok" || account?.balance?.remaining == null) return null;
	cache.dailyBalance ??= {};
	const today = balanceDayKey();
	let prev = null;
	for (const [day, snap] of Object.entries(cache.dailyBalance)) {
		if (day < today && (prev === null || day > prev.day)) prev = { day, ...snap };
	}
	if (prev === null) return null;
	const b = account.balance;
	const recharge = Math.max(0, (Number(b.breakdown?.toppedUp) || 0) - (Number(prev.toppedUp) || 0))
		+ Math.max(0, (Number(b.breakdown?.granted) || 0) - (Number(prev.granted) || 0));
	const estimate = (Number(prev.remaining) || 0) - (Number(b.remaining) || 0) - recharge;
	return {
		estimate: Math.max(0, Math.round(estimate * 100) / 100),
		fromDay: prev.day,
		toDay: today
	};
}
...
async function handleAccount(ctx, accounts, req, res) {
	...
	try {
		...
		const cache = await loadCache();
		noteDailyBalance(cache, account);
		account.allChannel = allChannelSpendEstimate(cache, account);
		await saveCache(ctx, cache);
...
async function apply(ctx, rawConfig = {}, deps = {}) {
	// 计价覆盖表（官方 V4 峰谷价目为缺省，见 lib/pricing.js）。
	priceConfig = rawConfig !== null && typeof rawConfig === "object"
		&& rawConfig.prices !== null && typeof rawConfig.prices === "object"
		? rawConfig.prices
		: {};
