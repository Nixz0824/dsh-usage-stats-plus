 ...
 * buckets and cost from the day/model bucket they were attributed to, so a
 * slice starting mid-step (e.g. a usage chunk at the tail of the previous
 * fold) stays exact.
 * @param state - session fold state (mutated in place).
 * @param events - the new events, in seq order, starting after the last fold.
 * @param price - cost function `(modelKey, timeMs, buckets) => cny`; exact
 *   cost is accumulated per sample with the same replace semantics as tokens.
 */
export function applyUsageDelta(state, events, price = () => 0) {
	...
	for (const event of events) {
		...
		const cost = Number(price(model, event.time, buckets)) || 0;
		...
		if (last !== null && last.key === sample.key) {
			...
			if (previous !== void 0) {
				...
				previous.cost = Math.max(0, (previous.cost ?? 0) - (last.cost ?? 0));
		...
		last = { key: sample.key, day, model, buckets, cost };
	}
...
export function mergeInto(byDay, sessionDays) {
	for (const [day, entry] of sessionDays) {
		...
		target.cost = (target.cost ?? 0) + (entry.cost ?? 0);
